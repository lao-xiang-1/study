# 词块整理

#en
## 一、intent / intense / intensely / intensive / intensity
?
### 1. intent
- **释义**：n. 意图，目的；adj. 专心的，专注的
- **例句**：
  - She was **intent** on finishing the project before the deadline.（她专心致志要在截止日期前完成项目。）
  - The original **intent** of the law was to protect consumers.（该法律的初衷是保护消费者。）

### 2. intense
- **释义**：adj. 强烈的，剧烈的；紧张的，认真的
- **例句**：
  - The heat was so **intense** that we had to stay indoors.（酷热难耐，我们只能待在室内。）
  - He is an **intense** competitor who never gives up.（他是一个极度认真的竞争者，从不放弃。）

### 3. intensely
- **释义**：adv. 强烈地；极度地；专注地
- **例句**：
  - She stared **intensely** at the screen, trying to find the error.（她紧盯着屏幕，试图找出错误。）
  - He was **intensely** focused on his research.（他极度专注于他的研究。）

### 4. intensive
- **释义**：adj. 密集的，集中的；深入细致的
- **例句**：
  - She took an **intensive** English course over the summer.（她暑假上了一个英语强化课程。）
  - The police launched an **intensive** investigation into the case.（警方对该案件展开了深入调查。）

### 5. intensity
- **释义**：n. 强度；强烈程度
- **例句**：
  - The storm is expected to increase in **intensity** overnight.（预计暴风雨夜间强度将增大。）
  - She spoke with great **intensity** about her passion for music.（她满怀激情地谈论对音乐的热爱。）

---

#en
## 二、stimulus / stimuli / stimulate / stimulation
?
### 1. stimulus
- **释义**：n. 刺激（物）；激励因素
- **例句**：
  - Light is a **stimulus** that causes the pupil to contract.（光线是引起瞳孔收缩的刺激物。）
  - The tax cut was designed as an economic **stimulus**.（减税政策旨在作为经济刺激手段。）

### 2. stimuli
- **释义**：n. stimulus 的复数形式
- **例句**：
  - The brain processes multiple sensory **stimuli** simultaneously.（大脑同时处理多种感官刺激。）
  - Plants respond to external **stimuli** such as light and gravity.（植物对外部刺激（如光和重力）做出反应。）

### 3. stimulate
- **释义**：v. 刺激，激发；促进
- **例句**：
  - Reading can **stimulate** a child's imagination.（阅读能激发孩子的想象力。）
  - The government hopes to **stimulate** economic growth.（政府希望刺激经济增长。）

### 4. stimulation
- **释义**：n. 刺激；激励；兴奋作用
- **例句**：
  - Babies need sensory **stimulation** for healthy development.（婴儿需要感官刺激来健康成长。）
  - The drug causes **stimulation** of the central nervous system.（该药物引起中枢神经系统的兴奋。）

---

#en
## 三、slight / slightly / slighted
?
### 1. slight
- **释义**：adj. 轻微的，微小的；纤细的
- **例句**：
  - There is a **slight** chance of rain this afternoon.（今天下午有轻微的可能下雨。）
  - She has a **slight** build but is very strong.（她身材纤细但非常强壮。）

### 2. slightly
- **释义**：adv. 轻微地，稍微
- **例句**：
  - The temperature dropped **slightly** overnight.（夜间气温略微下降。）
  - He was **slightly** annoyed by the delay.（他对延误感到有些恼怒。）

### 3. slighted
- **释义**：adj. 被轻视的，受冷落的（动词 slight 的过去分词：轻视、冷落）
- **例句**：
  - She felt **slighted** when she wasn't invited to the meeting.（没被邀请参加会议，她感到受了冷落。）
  - He **slighted** her contributions to the project.（他轻视了她对项目的贡献。）

---

#en
## 四、derive / derived / derivative
?
### 1. derive
- **释义**：v. 取得，获得；源自，起源于
- **例句**：
  - He **derives** great satisfaction from helping others.（他从帮助他人中获得极大的满足。）
  - Many English words are **derived** from Latin.（许多英语单词源自拉丁语。）

### 2. derived
- **释义**：adj. 派生的，衍生的
- **例句**：
  - The **derived** data was used to train the model.（派生数据被用于训练模型。）
  - This is a **derived** form of the original equation.（这是原始方程的推导形式。）

### 3. derivative
- **释义**：n. 派生物，衍生物；导数；adj. 缺乏原创性的
- **例句**：
  - The **derivative** of x² is 2x.（x² 的导数是 2x。）
  - Critics called the film **derivative** and uninspired.（评论家称这部电影缺乏原创性，毫无新意。）

---

#en
## 五、precedent / precedence / unprecedented
?
### 1. precedent
- **释义**：n. 先例，前例；adj. 在先的
- **例句**：
  - The court's ruling set a legal **precedent** for future cases.（法院的裁决为今后的案件树立了法律先例。）
  - There is no **precedent** for such a decision in this country.（这个国家没有此类决定的先例。）

### 2. precedence
- **释义**：n. 优先权，优先地位
- **例句**：
  - Safety must take **precedence** over cost considerations.（安全必须优先于成本考量。）
  - Ambulances have **precedence** over other vehicles on the road.（救护车在道路上享有优先通行权。）

### 3. unprecedented
- **释义**：adj. 史无前例的，前所未有的
- **例句**：
  - The pandemic caused **unprecedented** disruption to the global economy.（疫情对全球经济造成了前所未有的冲击。）
  - The team achieved an **unprecedented** level of success this season.（该团队本赛季取得了史无前例的成就。）

---

#en
## 六、specification / specified / specify
?
### 1. specification
- **释义**：n. 规格，规范；详细说明
- **例句**：
  - The product must meet the technical **specifications** outlined in the contract.（产品必须符合合同中列出的技术规格。）
  - Detailed **specifications** were provided for the new building project.（新建筑项目提供了详细的规范说明。）

### 2. specified
- **释义**：adj. 指定的，明确规定的
- **例句**：
  - Please submit your report by the **specified** deadline.（请在规定的截止日期前提交报告。）
  - Only the **specified** materials may be used in the experiment.（实验中只能使用指定的材料。）

### 3. specify
- **释义**：v. 具体说明，明确规定
- **例句**：
  - The contract **specifies** that payment is due within 30 days.（合同明确规定付款须在30天内完成。）
  - Please **specify** your preferred delivery address when ordering.（订购时请明确您的收货地址。）

---

#en
## 七、deceive / deception / deceptive
?
### 1. deceive
- **释义**：v. 欺骗，蒙蔽
- **例句**：
  - He tried to **deceive** his parents about where he had been.（他试图对父母隐瞒自己的去向。）
  - Don't be **deceived** by its harmless appearance.（不要被它无害的外表所欺骗。）

### 2. deception
- **释义**：n. 欺骗，诡计，骗局
- **例句**：
  - The investigation revealed a pattern of lies and **deception**.（调查揭露了一系列谎言和欺骗行为。）
  - She felt guilty about the **deception** she had practiced.（她对自己的欺骗行为感到内疚。）

### 3. deceptive
- **释义**：adj. 欺骗性的，误导的
- **例句**：
  - The advertisement was found to be **deceptive** and was pulled from television.（该广告被认定具有欺骗性，已从电视上撤下。）
  - The calm surface of the lake is **deceptive** — the currents below are dangerous.（湖面看似平静，实则暗流涌动。）

---

#en
## 八、reception / receptionist / receptive
?
### 1. reception
- **释义**：n. 接待，招待会；接收（信号等）；反响
- **例句**：
  - The wedding **reception** was held in a beautiful garden.（婚宴在一座美丽的花园举行。）
  - The new policy received a warm **reception** from the public.（新政策受到了公众的热烈欢迎。）

### 2. receptionist
- **释义**：n. 接待员，前台
- **例句**：
  - The **receptionist** greeted every visitor with a friendly smile.（前台接待员以友好的微笑迎接每位访客。）
  - She works as a **receptionist** at a large law firm.（她在一家大型律师事务所担任前台接待。）

### 3. receptive
- **释义**：adj. 乐于接受的，善于接纳的
- **例句**：
  - The audience was very **receptive** to the speaker's ideas.（听众非常乐于接受演讲者的观点。）
  - Children are generally more **receptive** to learning new languages.（儿童通常更善于学习新语言。）

---

#en
## 九、emerge / emerging
?
### 1. emerge
- **释义**：v. 出现，浮现；脱颖而出；（事实）暴露
- **例句**：
  - New evidence **emerged** during the trial that changed the outcome.（审判期间出现了改变结果的新证据。）
  - China has **emerged** as a leading force in renewable energy.（中国已崛起为可再生能源领域的领军力量。）

### 2. emerging
- **释义**：adj. 新兴的，初露头角的
- **例句**：
  - **Emerging** technologies like AI are transforming every industry.（人工智能等新兴技术正在改变各行各业。）
  - The fund focuses on investing in **emerging** markets.（该基金专注于投资新兴市场。）

---

#en
## 十、interpret / interpretability
?
### 1. interpret
- **释义**：v. 解释，理解；口译
- **例句**：
  - How do you **interpret** the results of this experiment?（你如何解读这个实验的结果？）
  - She was hired to **interpret** at the international conference.（她被雇来在国际会议上担任口译。）

### 2. interpretability
- **释义**：n. 可解释性，可理解性
- **例句**：
  - Model **interpretability** is crucial in medical AI applications.（模型的可解释性在医疗AI应用中至关重要。）
  - Researchers are working to improve the **interpretability** of deep learning systems.（研究人员正致力于提高深度学习系统的可解释性。）

---

#en
## 十一、equivalent / equivalence
?
### 1. equivalent
- **释义**：adj. 等价的，相当的；n. 等价物，对等物
- **例句**：
  - One mile is roughly **equivalent** to 1.6 kilometers.（一英里大约等于1.6公里。）
  - The Japanese word has no direct **equivalent** in English.（这个日语词在英语中没有直接对应的词。）

### 2. equivalence
- **释义**：n. 等价，相等；对等关系
- **例句**：
  - The **equivalence** of mass and energy is expressed by E=mc².（质能等价关系由 E=mc² 表达。）
  - The two qualifications are recognized as having **equivalence**.（这两种资格被认定为具有同等效力。）

---

#en
## 十二、procrastinate / procrastinator
?
### 1. procrastinate
- **释义**：v. 拖延，耽搁
- **例句**：
  - Stop **procrastinating** and start studying for the exam.（别再拖了，赶紧复习考试吧。）
  - He tends to **procrastinate** when faced with difficult tasks.（面对困难任务时他往往会拖延。）

### 2. procrastinator
- **释义**：n. 拖延者，拖延症患者
- **例句**：
  - As a chronic **procrastinator**, she always leaves assignments until the last minute.（作为一个习惯性拖延者，她总是把作业拖到最后一刻才做。）
  - The app is designed to help **procrastinators** stay on track.（这款应用旨在帮助拖延者保持进度。）

---

#en
## 十三、recursive / recursion
?
### 1. recursive
- **释义**：adj. 递归的，循环的
- **例句**：
  - A **recursive** function calls itself to solve smaller subproblems.（递归函数通过调用自身来求解更小的子问题。）
  - The directory structure is **recursive**, with folders nested inside folders.（目录结构是递归的，文件夹嵌套在文件夹中。）

### 2. recursion
- **释义**：n. 递归，递归法
- **例句**：
  - **Recursion** is a fundamental concept in computer science and mathematics.（递归是计算机科学和数学中的一个基本概念。）
  - The algorithm uses **recursion** to traverse the tree data structure.（该算法使用递归来遍历树形数据结构。）

---

#en
## 十四、variable / variance
?
### 1. variable
- **释义**：n. 变量；adj. 可变的，易变的
- **例句**：
  - Temperature is the independent **variable** in this experiment.（温度是本实验中的自变量。）
  - Weather conditions are highly **variable** in mountainous regions.（山区的天气状况变化无常。）

### 2. variance
- **释义**：n. 方差，变异数；分歧，差异
- **例句**：
  - A high **variance** indicates that the data points are spread far from the mean.（高方差表明数据点远离均值分布。）
  - There is considerable **variance** in opinion among the committee members.（委员会成员之间意见分歧很大。）

---

#en
## 十五、obsess / obsessive
?
### 1. obsess
- **释义**：v. 使着迷，使念念不忘
- **例句**：
  - He **obsesses** over every detail of his presentation.（他对演讲的每一个细节都 obsess 不已。）
  - Don't let one failure **obsess** you — move on and learn from it.（不要让一次失败占据你的全部心思——向前看，从中学习。）

### 2. obsessive
- **释义**：adj. 强迫性的，过分执着的；n. 强迫症患者
- **例句**：
  - Her **obsessive** attention to detail made her an excellent editor.（她对细节的执着追求使她成为一名出色的编辑。）
  - He has an **obsessive** interest in collecting vintage watches.（他对收集古董手表有着近乎痴迷的兴趣。）

---

#en
## 十六、complain / complaint
?
### 1. complain
- **释义**：v. 抱怨，发牢骚；投诉
- **例句**：
  - Customers frequently **complain** about the slow service at this restaurant.（顾客经常抱怨这家餐厅服务太慢。）
  - He **complained** to the manager that the room was too noisy.（他向经理投诉房间太吵了。）

### 2. complaint
- **释义**：n. 抱怨，投诉；疾病
- **例句**：
  - The company received over 200 **complaints** about the defective product.（公司收到了200多起关于该缺陷产品的投诉。）
  - She filed a formal **complaint** with the human resources department.（她向人力资源部门提交了正式投诉。）

---

#en
## 十七、synchronize / sync
?
### 1. synchronize
- **释义**：v. 使同步，使协调；校准（时间）
- **例句**：
  - We need to **synchronize** our watches before the mission begins.（任务开始前我们需要校准手表时间。）
  - The software can **synchronize** files across multiple devices automatically.（该软件可以自动在多个设备之间同步文件。）

### 2. sync
- **释义**：n./v. 同步（synchronize 的缩写）
- **例句**：
  - Make sure your calendar is in **sync** with your team's schedule.（确保你的日历与团队的日程保持同步。）
  - The audio and video are out of **sync** — the lips don't match the words.（音视频不同步——口型对不上台词。）

---

#en
## 十八、iterative / iterable
?
### 1. iterative
- **释义**：adj. 迭代的，重复的
- **例句**：
  - Software development often follows an **iterative** process of design, build, and test.（软件开发通常遵循设计、构建、测试的迭代过程。）
  - The team uses **iterative** refinement to gradually improve the product.（团队使用迭代优化来逐步改进产品。）

### 2. iterable
- **释义**：adj. 可迭代的（编程术语）
- **例句**：
  - In Python, lists and tuples are **iterable** objects that can be used in for loops.（在 Python 中，列表和元组是可用于 for 循环的可迭代对象。）
  - Any object that implements the `__iter__` method is considered **iterable**.（任何实现了 `__iter__` 方法的对象都被视为可迭代的。）

---

#en
## 十九、invoke / invocation
?
### 1. invoke
- **释义**：v. 调用；援引；祈求
- **例句**：
  - The program **invokes** a system call to allocate memory.（程序调用系统调用来分配内存。）
  - He **invoked** his right to remain silent.（他援引了自己保持沉默的权利。）

### 2. invocation
- **释义**：n. 调用；祈求；咒语
- **例句**：
  - The **invocation** of the remote API takes less than 50 milliseconds.（远程 API 的调用耗时不到 50 毫秒。）
  - The priest's **invocation** blessed the congregation before the ceremony.（牧师的祈祷在仪式前祝福了会众。）

---

#en
## 二十、alternate / alternative
?
### 1. alternate
- **释义**：v. 交替；轮流 adj. 交替的；间隔的
- **例句**：
  - The weather **alternates** between sunshine and rain throughout the week.（这一周天气在晴天和雨天之间交替变化。）
  - She used **alternate** layers of chocolate and cream in the cake.（她在蛋糕中使用了巧克力和奶油交替的层。）

### 2. alternative
- **释义**：n. 替代方案；选择 adj. 替代的；另类的
- **例句**：
  - Public transport is a viable **alternative** to driving in the city.（公共交通是在城市中驾车出行的可行替代方案。）
  - The committee proposed an **alternative** plan to reduce costs.（委员会提出了一项降低成本的替代方案。）

---

#en
## 二十一、surgery / surgically
?
### 1. surgery
- **释义**：n. 外科手术；手术室；诊所
- **例句**：
  - The patient was admitted to the hospital for heart **surgery**.（患者被送入医院进行心脏手术。）
  - Dr. Lee has performed over two thousand **surgeries** in her career.（李医生在她的职业生涯中已完成了超过两千台手术。）

### 2. surgically
- **释义**：adv. 外科手术般地；精确地
- **例句**：
  - The tumor was **surgically** removed without complications.（肿瘤被手术切除，未出现并发症。）
  - The special forces team moved **surgically** through the building to neutralize the threat.（特种部队如外科手术般精准地穿过大楼消除了威胁。）

---

#en
## 二十二、declare / declarative
?
### 1. declare
- **释义**：v. 声明；宣布；申报
- **例句**：
  - The government **declared** a state of emergency after the earthquake.（政府在地震后宣布了紧急状态。）
  - You must **declare** all income on your tax return.（你必须在纳税申报表中申报所有收入。）

### 2. declarative
- **释义**：adj. 声明式的；陈述的
- **例句**：
  - SQL is a **declarative** language — you specify what result you want, not how to compute it.（SQL 是一种声明式语言——你指定想要的结果，而非如何计算它。）
  - A **declarative** sentence simply states a fact and ends with a period.（陈述句只是陈述一个事实，以句号结尾。）

---

#en
## 二十三、convention / conventional
?
### 1. convention
- **释义**：n. 惯例；公约；大会
- **例句**：
  - By **convention**, variable names in Python use snake_case.（按照惯例，Python 中的变量名使用蛇形命名法。）
  - Delegates from forty countries attended the climate **convention**.（来自四十个国家的代表参加了气候大会。）

### 2. conventional
- **释义**：adj. 传统的；常规的；惯例的
- **例句**：
  - **Conventional** farming methods rely heavily on chemical fertilizers.（传统农业方法严重依赖化学肥料。）
  - She chose a **conventional** approach to solving the problem rather than experimenting.（她选择了常规的方法来解决问题，而非进行实验。）

---

#en
## 二十四、racial / race
?
### 1. racial
- **释义**：adj. 种族的；种族间的
- **例句**：
  - The organization campaigns against **racial** discrimination in the workplace.（该组织开展反对职场种族歧视的运动。）
  - **Racial** diversity enriches a community with different perspectives and traditions.（种族多样性以不同的视角和传统丰富了社区。）

### 2. race
- **释义**：n. 种族；赛跑 v. 赛跑；急速移动
- **例句**：
  - The Olympic 100-metre **race** is one of the most watched events in the world.（奥运会百米赛跑是世界上最受关注的项目之一。）
  - People of every **race** deserve equal rights and opportunities.（每个种族的人都应享有平等的权利和机会。）

---

#en
## 二十五、extrovert / introvert
?
### 1. extrovert
- **释义**：n. 外向的人 adj. 外向的
- **例句**：
  - As an **extrovert**, she thrives in social gatherings and large parties.（作为一个外向的人，她在社交聚会和大型派对中如鱼得水。）
  - **Extroverts** tend to recharge their energy by interacting with others.（外向的人往往通过与他人的互动来恢复精力。）

### 2. introvert
- **释义**：n. 内向的人 adj. 内向的
- **例句**：
  - An **introvert** may find prolonged social interaction draining and need time alone to recover.（内向的人可能觉得长时间的社交令人疲惫，需要独处的时间来恢复。）
  - Being an **introvert** does not mean you are shy; it simply means you prefer quieter environments.（内向并不意味着害羞，只是你更喜欢安静的环境。）

---

#en
## 二十六、assign / assignment
?
### 1. assign
- **释义**：v. 分配；指派；赋值
- **例句**：
  - The teacher **assigned** three chapters of reading for homework.（老师布置了三章阅读作为家庭作业。）
  - Each process is **assigned** a unique identifier by the operating system.（操作系统为每个进程分配一个唯一的标识符。）

### 2. assignment
- **释义**：n. 作业；任务；分配；赋值
- **例句**：
  - The final **assignment** is due at the end of the semester.（期末作业在学期末截止。）
  - Her first **assignment** as a journalist was to cover the city council election.（她作为记者的第一个任务是报道市议会选举。）

---

#en
## 二十七、compress / compressed
?
### 1. compress
- **释义**：v. 压缩；精简 n. 敷布
- **例句**：
  - Use gzip to **compress** the log files before archiving them.（在归档前使用 gzip 压缩日志文件。）
  - The editor managed to **compress** a long report into a concise summary.（编辑成功地将一篇长报告压缩成了简洁的摘要。）

### 2. compressed
- **释义**：adj. 压缩的；紧凑的
- **例句**：
  - **Compressed** air is used to power pneumatic tools on the assembly line.（压缩空气用于驱动装配线上的气动工具。）
  - The **compressed** schedule left the team only two weeks to finish the project.（紧凑的时间表只给团队留了两周来完成项目。）

---

#en
## 二十八、paradox / paradoxical
?
### 1. paradox
- **释义**：n. 悖论；矛盾的事物
- **例句**：
  - It is a **paradox** that the richest country in the world still has millions living in poverty.（世界上最富有的国家仍有数百万人生活在贫困中，这是一个悖论。）
  - The liar **paradox** — "This statement is false" — has puzzled logicians for centuries.（说谎者悖论——"这句话是假的"——困扰了逻辑学家几个世纪。）

### 2. paradoxical
- **释义**：adj. 矛盾的；悖论的
- **例句**：
  - It seems **paradoxical** that drinking too much water can actually dehydrate you.（喝太多水实际上会让你脱水，这似乎很矛盾。）
  - The study revealed a **paradoxical** result: stricter regulations led to faster innovation.（该研究揭示了一个矛盾的结果：更严格的法规反而促进了更快的创新。）

---

#en
## 二十九、statistic / stats
?
### 1. statistic
- **释义**：n. 统计量；统计数据
- **例句**：
  - The unemployment **statistic** released today surprised many economists.（今天公布的失业统计数据令许多经济学家感到意外。）
  - Each **statistic** in the dataset represents the average score of a different age group.（数据集中的每个统计量代表不同年龄组的平均分数。）

### 2. stats
- **释义**：n.（statistics 的缩写）统计数据；统计学
- **例句**：
  - The player's **stats** this season include 25 goals and 12 assists.（该球员本赛季的数据包括 25 个进球和 12 次助攻。）
  - I need to study for my **stats** exam next Monday.（我需要为下周一的统计学考试复习。）

---

#en
## 三十、cortex / cortical
?
### 1. cortex
- **释义**：n. 皮层；大脑皮层
- **例句**：
  - The prefrontal **cortex** plays a key role in decision-making and planning.（前额叶皮层在决策和规划中起着关键作用。）
  - The visual **cortex** at the back of the brain processes information from the eyes.（大脑后部的视觉皮层处理来自眼睛的信息。）

### 2. cortical
- **释义**：adj. 皮层的；皮质的
- **例句**：
  - **Cortical** thickness can be measured using high-resolution MRI scans.（皮层厚度可以通过高分辨率 MRI 扫描来测量。）
  - The study found that **cortical** neurons fire in distinct patterns during sleep.（研究发现皮层神经元在睡眠期间以不同的模式放电。）

---

#en
## 三十一、fragment / defragment
?
### 1. fragment
- **释义**：n. 碎片；片段 v. 使破碎；使分裂
- **例句**：
  - Archaeologists discovered a **fragment** of ancient pottery at the excavation site.（考古学家在发掘现场发现了一片古代陶器碎片。）
  - Over time, the hard drive became **fragmented**, slowing down file access.（随着时间的推移，硬盘变得碎片化，文件访问速度变慢。）

### 2. defragment
- **释义**：v. 整理碎片；磁盘碎片整理
- **例句**：
  - You should **defragment** your mechanical hard drive periodically to improve performance.（你应该定期整理机械硬盘的碎片以提高性能。）
  - Modern operating systems **defragment** drives automatically in the background.（现代操作系统会在后台自动整理磁盘碎片。）

---

#en
## 三十二、sensory / sensible
?
### 1. sensory
- **释义**：adj. 感觉的；感官的
- **例句**：
  - **Sensory** neurons transmit signals from the skin to the brain.（感觉神经元将信号从皮肤传递到大脑。）
  - The **sensory** overload of bright lights and loud music made it hard to concentrate.（强光和嘈杂音乐的感官过载使人难以集中注意力。）

### 2. sensible
- **释义**：adj. 明智的；合理的；实用的
- **例句**：
  - It is **sensible** to wear a helmet when riding a bicycle.（骑自行车时戴头盔是明智的。）
  - She made a **sensible** decision to save money instead of spending it on luxuries.（她做了一个明智的决定，把钱存起来而不是花在奢侈品上。）

---

#en
## 三十三、grant / granted
?
### 1. grant
- **释义**：v. 授予；批准；承认 n. 拨款；补助金
- **例句**：
  - The university **granted** her a full scholarship for her doctoral studies.（大学授予她全额奖学金攻读博士学位。）
  - The research team received a **grant** of two million dollars from the National Science Foundation.（研究团队从国家科学基金会获得了两百万美元的拨款。）

### 2. granted
- **释义**：adj. 理所当然的（常用于 take for granted）conj. 诚然；姑且承认
- **例句**：
  - We often take clean drinking water for **granted**, but billions of people lack access to it.（我们常常把清洁饮用水视为理所当然，但数十亿人无法获得它。）
  - **Granted**, the new system is more expensive, but it is also far more reliable.（诚然，新系统更昂贵，但它也可靠得多。）

---

#en
## 三十四、transition / transitive
?
### 1. transition
- **释义**：n. 过渡；转变 v. 过渡；转变
- **例句**：
  - The **transition** from fossil fuels to renewable energy will take decades.（从化石燃料到可再生能源的过渡将需要数十年。）
  - The company is **transitioning** to a fully remote work model.（公司正在过渡到完全远程办公的模式。）

### 2. transitive
- **释义**：adj. 传递的；及物的
- **例句**：
  - In the sentence "She reads books," the verb "reads" is **transitive** because it takes a direct object.（在句子"She reads books"中，动词"reads"是及物的，因为它带有直接宾语。）
  - Equality is a **transitive** relation: if A equals B and B equals C, then A equals C.（等价是一种传递关系：如果 A 等于 B 且 B 等于 C，那么 A 等于 C。）

---

#en
## 三十五、costume / custom
?
### 1. costume
- **释义**：n. 服装；戏装；套装
- **例句**：
  - Children dressed in **costumes** went trick-or-treating on Halloween night.（孩子们穿上戏装在万圣节前夜挨家挨户要糖果。）
  - The museum displayed traditional **costumes** worn by the royal court in the 18th century.（博物馆展出了 18 世纪皇室穿着的传统服饰。）

### 2. custom
- **释义**：n. 习俗；惯例；海关 adj. 定制的
- **例句**：
  - It is a **custom** in many cultures to remove your shoes before entering a home.（在许多文化中，进门前脱鞋是一种习俗。）
  - He ordered a **custom**-built laptop with extra memory and a larger screen.（他订购了一台定制笔记本电脑，配有额外的内存和更大的屏幕。）

---

#en
## 三十六、slot / time slot
?
### 1. slot
- **释义**：n. 狭槽；位置；时段 v. 把……放入狭槽；安排
- **例句**：
  - Insert the coin into the **slot** to start the vending machine.（把硬币投入投币口来启动自动售货机。）
  - The new TV show has been given a prime-time **slot** on Friday evenings.（这档新电视节目被安排在周五晚间的黄金时段。）

### 2. time slot
- **释义**：n. 时间段；时段
- **例句**：
  - Please select an available **time slot** for your appointment next week.（请选择下周预约的可用时间段。）
  - The conference schedule has a 15-minute **time slot** for each presenter.（会议日程为每位演讲者安排了 15 分钟的时间段。）
