...
该电路是一个**共集电极放大电路**（也叫射极跟随器），因为输入信号从基极输入，输出信号从发射极取出，集电极接交流地。

### （1）求静态工作点 $I_{CQ}$ 和 $U_{CEQ}$
![](assets/三极管例题2.png)
进行直流静态分析时，将所有的电容（$C_{b1}$, $C_{b2}$）视为开路。画出直流等效电路

基极电路由 $R_{b1}$ 和 $R_{b2}$ 组成分压偏置。为了简化计算，我们利用**戴维南定理**将基极电路等效为一个电压源 $V_{BB}$ 和一个内阻 $R_B$ 的串联。
*这里也可以列方程求解 --tz*

*   **等效基极电源电压 $V_{BB}$：**（将输出端视为开路）
...
*   **等效基极电阻 $R_B$：**（无视电压源作用，等效于$V_{CC}$接地）
...
从基极看进去，三极管本身的输入电阻为：$R_{ib} = r_{be} + (1 + \beta)R_L'$, 参考[晶体管内部的阻抗变换原理](../晶体管内部的阻抗变换原理.md)
...
$$\dot{A}_u = \frac{\dot{U}_o}{\dot{U}_i} = \frac{(1+\beta) \cdot R_L'}{R_{ib}} = \frac{(1 + \beta)R_L'}{r_{be} + (1 + \beta)R_L'}$$
代入得：
$$\dot{A}_u = \frac{134.63}{136.68} \approx \mathbf{0.985}$$
