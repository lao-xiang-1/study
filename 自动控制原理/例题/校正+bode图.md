...
### 1. 写出严格的幅值表达式
校正后的开环传递函数为：
$$G_{open}(j\omega) = \frac{20(1 + 2j\omega)}{j\omega(1 + 10j\omega)(1 + 0.1j\omega)}$$

对其取模，得到严格的幅频特性表达式：
$$|G_{open}(j\omega)| = \frac{20 \sqrt{1 + (2\omega)^2}}{\omega \sqrt{1 + (10\omega)^2} \sqrt{1 + (0.1\omega)^2}}$$

### 2. 确定所处的频段
在第（2）问中，我们根据转折频率画出了渐近线伯德图：
*   极点：$\omega_1 = 0.1$
*   零点：$\omega_2 = 0.5$
*   极点：$\omega_3 = 10$

因为我们要找截止频率 $\omega_{c2}$（也就是幅值为 1，图上纵坐标为 0dB 的点），从画出的草图中我们可以判断出，穿越 0dB 线的点位于 **中频段 $0.5 < \omega < 10$** 之间。

### 3. 在 $0.5 < \omega < 10$ 频段内进行各项近似
伯德图渐近线的核心思想是：**对于典型环节 $(1 + jT\omega)$，当频率远大于转折频率时近似为 $T\omega$；当频率远小于转折频率时近似为 $1$。**

我们针对频段 $0.5 < \omega < 10$ 来分别看三个括号里的项：

*   **对于零点 $(1 + 2j\omega)$：** 
    转折频率是 $0.5$。既然当前 $\omega > 0.5$，说明我们处于该环节的高频区，此时 $(2\omega)^2 \gg 1$。
    所以近似为：**$\sqrt{1 + (2\omega)^2} \approx \sqrt{(2\omega)^2} = 2\omega$**

*   **对于极点 $(1 + 10j\omega)$：** 
    转折频率是 $0.1$。既然当前 $\omega > 0.5 > 0.1$，说明我们也处于该环节的高频区，此时 $(10\omega)^2 \gg 1$。
    所以近似为：**$\sqrt{1 + (10\omega)^2} \approx \sqrt{(10\omega)^2} = 10\omega$**

*   **对于极点 $(1 + 0.1j\omega)$：** 
    转折频率是 $10$。既然当前 $\omega < 10$，说明我们处于该环节的低频区，此时 $(0.1\omega)^2$ 相对较小。
    所以在渐近线处理中近似为：**$\sqrt{1 + (0.1\omega)^2} \approx \sqrt{1} = 1$**

### 4. 代入表达式得到结果
将上述三个近似结果代回最开始的严格幅值表达式中：
$$|G_{open}(j\omega)| \approx \frac{20 \cdot (2\omega)}{\omega \cdot (10\omega) \cdot (1)}$$
化简一下：
$$|G_{open}(j\omega)| \approx \frac{40\omega}{10\omega^2} = \frac{4}{\omega}$$

---
**总结：**
这种近似方法其实就是写出**该段渐近线的直线方程**对应的实际表达式。因为我们在 $0.5 \sim 10$ 这段频率范围内找穿越点，所以把频率范围内不起主导作用的项省略掉（高频项把 1 忽略，低频项把 $\omega$ 忽略），就得到了 $\frac{4}{\omega}$ 这个非常简单的代数式，令其等于 1 就能瞬间算出 $\omega_{c2} = 4\text{ rad/s}$。
