...
>同样，只有在`[project] dependencies`中指定依赖包，才会在`uv sync`中生效。`tool.uv.sources`只是让依赖包从**特定位置**安装，而不从`pypi`下载，
...
> *具体语法*：在`[tool.uv.sources]`中指定 sources 如`index`，然后在`[[tool.uv.index]]`中具体说明 url 等信息。

> [!tip] Tip
> 可以用`uv add`快速添加，也可以在`pyproject.toml`中手动指定。
